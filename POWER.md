...
Depot provides a remote container build service that accelerates container builds with optimized construction of layers and automatic incremental caching. It is a drop-in replacement for `docker buildx build`, but instead of performing the build locally, it transfers the build context to one or more remote build servers running in the cloud.

After an image is built, it is stored in the Depot cache. The user can also choose to pass the `--load` flag to load the image into the local Docker daemon, or the `--push` flag to push the image to a registry. If the user wishes to push to Depot's registry, the `--save` flag enables this.
...
**IMPORTANT**: You should always use the `depot` CLI instead of the equivalent `docker` or `docker buildx` commands to benefit from Depot's caching and optimizations.
...
When building an image, testing a Dockerfile locally, etc, you should always use the `depot build` or `depot bake` commands instead of the equivalent Docker commands:
...
- [Privacy Policy](https://depot.dev/privacy-policy)
